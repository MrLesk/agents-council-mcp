    ...
    for (const block of message.message.content) {
      if (block.type === "tool_use" && block.name === "TodoWrite") {
        ...
        todos.forEach((todo, index) => {
          const status = todo.status === "completed" ? "[ok]" :
                        todo.status === "in_progress" ? "[tool]" : "[x]";
        ...
        for block in message.content:
            if isinstance(block, ToolUseBlock) and block.name == "TodoWrite":
                ...
                for i, todo in enumerate(todos):
                    status = "[ok]" if todo["status"] == "completed" else \
                            "[tool]" if todo["status"] == "in_progress" else "[x]"
...
class TodoTracker {
  ...
  displayProgress() {
    ...
    this.todos.forEach((todo, index) => {
      const icon = todo.status === "completed" ? "[ok]" : 
                  todo.status === "in_progress" ? "[tool]" : "[x]";
...
class TodoTracker:
    ...
    def display_progress(self):
        ...
        for i, todo in enumerate(self.todos):
            icon = "[ok]" if todo["status"] == "completed" else \
                  "[tool]" if todo["status"] == "in_progress" else "[x]"
